...
In `src/shared/types.ts`, update `TriggerContext` (use optional to avoid breaking existing call sites before Task 7 wires it):
...
**IMPORTANT: Execution order matters.** Insert badge evaluation BEFORE the `firstApiCallSeen` flip at line 68 and BEFORE the daily history update. This ensures `badge_first_call` sees `firstApiCallSeen: false` on the first API call. Insert after the quote trigger evaluation block (after line 66), before line 68:

Also increase the `dailyHistory` cap from 14 to 30 at line 81 to support `badge_streak_30`:

```typescript
  dailyHistory = dailyHistory.slice(-30); // was -14, increased for badge_streak_30
```

Badge evaluation code to insert:
...
Replace the entire Collection component with the following complete implementation:
...
**IMPORTANT:** Do NOT register a new `SETTINGS_SET` handler. Instead, add skin save logic INSIDE the existing `SETTINGS_SET` handler at `ipc-handlers.ts:36-50`. Add after the `autoStart` sync block (after line 44) and before `onSettingsChanged?.()`:
...
Add the following imports and state to Settings.tsx:

```typescript
import { SkinConfig, SkinMode, MamaMood, MamaErrorExpression } from '../../shared/types';
```

Add skin state inside the component (after existing state declarations):
...
```typescript
  ...
  const handleSkinUpload = async (mood?: string) => {
    const path = await window.electronAPI.uploadSkin(mood);
    if (!path) return;

    if (skinConfig.mode === 'single') {
      setSkinConfig((prev) => ({ ...prev, singleImagePath: path }));
    } else if (skinConfig.mode === 'per-mood' && mood) {
      setSkinConfig((prev) => ({
        ...prev,
        moodImages: { ...prev.moodImages, [mood]: path },
      }));
    } else if (skinConfig.mode === 'spritesheet') {
      setSkinConfig((prev) => ({
        ...prev,
        spritesheet: { ...prev.spritesheet!, imagePath: path },
      }));
    }
  };
...
Add the following JSX after the Always on Top card in the settings tab:
...
Also add these new style entries to the `s` object:
...
```typescript
  ...
  numInput: {
    width: '100%', padding: '6px 8px', fontSize: 13, border: '1px solid #e5e7eb',
    borderRadius: 6, marginTop: 4, textAlign: 'center',
  },
```
