...
| `messages.ts` | `MESSAGE_POOLS` map (locale → mood → string[]) and `getMessage()` with round-robin rotation; exposes `getCurrentCommonQuoteId()` for collection tracking |
...
- `computeMood()` in `mood-engine.ts` is a pure function: given the same input it always returns the same output. Keep it that way — no global state, no timers.
